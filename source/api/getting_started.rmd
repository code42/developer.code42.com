...
Only use [API clients](#tag/Auth/paths/~1v1~1oauth/post) for authentication. All previous API authentication methods are deprecated and are obsolete as of September 2022:
...
**Basic authentication to obtain an authentication token with the auth/jwt method:**

```bash
curl -u "<username>:<password>"  https://<RequestURL>/c42api/v3/auth/jwt?useBody=true
```

**Basic authentication with second factor (TOTP) token:**
